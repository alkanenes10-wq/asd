...
- Convert it to `oklch(...)` and update `--color-primary`, `--color-accent`,
  `--color-ring` and `--color-sidebar-active` in the `@theme` block of
  `app/globals.css`. The default is a coral-pink `oklch(68% 0.15 10)`.
- The sidebar is white (a clean light shell) — leave `--color-sidebar` as is.
- Quick recipe: pick the hue (H) of their color and reuse the existing L/C, e.g.
  emerald ≈ `oklch(62% 0.15 152)`, blue ≈ `oklch(60% 0.16 255)`,
  coral ≈ `oklch(68% 0.15 10)`, violet ≈ `oklch(58% 0.18 295)`,
  amber ≈ `oklch(72% 0.14 70)`. You can also nudge the service swatches
  (`--svc-*`) if you want the schedule colors to feel on-brand.
